...
private def treeA_benchmark : IO Unit := do
  ...
  let trustClaim : TrustClaim := .trust .oneSample t n_white Denied f_white p_benchmark ci σ
...
private def treeB_temporal : IO Unit := do
  ...
  -- IUT2 node (depth 3)
  let utrustClaim : TrustClaim := .untrust .twoSample tB nBpool Denied wfB wfW ci (σB ∪ σW)
...
/-
  ...
  Tree (depth 4):
    ...
    ══════════════════════════════════════ EUT
    {dB : Denied_{¬[ℓ,h]}} ⊢ dB_{nB} : Denied_{fB}  [freq]
-/
...
private def treeC_fullChain : IO Unit := do
  ...
  -- IEx: the pooled Black rate significantly EXCEEDS the pooled White rate
  -- (0 ∉ CI).  Two-sample certificates eliminate through the comparison path
  -- (EEx), not EUT, so the extraction re-enters via a shifted benchmark.
  let leftTC  : TermClaim := ⟨.frequency, tB, nBpool, Denied, wfB, σB⟩
  let rightTC : TermClaim := ⟨.frequency, tW, nWpool, Denied, wfW, σW⟩
  match probSub wfB wfW, ci with
  | some diff, .interval lo hi => do
    let excessClaim : ComparisonClaim := .excess leftTC rightTC diff ci
    let dIEx := nd "IEx" [dUpdateB, dUpdateW] (obsCtxB ++ obsCtxW)
      (.comparison excessClaim)

    -- EEx: take the White pooled rate as the benchmark p and shift it by the
    -- certified excess [ℓ,h] to bound the Black rate in [p+ℓ, p+h] (depth 4).
    let modelEntry : ContextEntry :=
      ⟨"pW", mkSupport "WhiteDE", Denied, .exact wfW⟩
    let dModel := nd "identity" [] [modelEntry] (.identity modelEntry)
    let shiftEntry : ContextEntry :=
      ⟨"dB", mkSupport "DE_lending", Denied,
        .interval (clampProb (wfW.val + lo.val)) (clampProb (wfW.val + hi.val))⟩
    let dEEx := nd "EEx" [dIEx, dModel]
      (obsCtxB ++ obsCtxW ++ [modelEntry, shiftEntry]) (.term leftTC)

    IO.println s!"  Full chain: obs → UPDATE → IEx → EEx"
    ...
    IO.println s!"  Excess CI (Black − White): {showConstraint ci}"
    IO.println s!"  EEx bounds the Black rate in [pW+ℓ, pW+h] from the benchmark"
    IO.println ""

    runTest "Tree C: Full chain UPDATE→IEx→EEx (depth 4)" dEEx

    IO.println ""
    IO.println "  ┌─────────────────────────────────────────────────────────────┐"
    IO.println "  │  (b)(d) The extracted term claim is a transferable artifact │"
    IO.println "  │  carrying the data plus the benchmark-shifted bound on the  │"
    IO.println "  │  Black rate in its typing context.                          │"
    IO.println "  │                                                            │"
    ...
    IO.println ""
    renderTree dEEx "  "
  | _, _ => IO.println "  SKIP: CI is not an interval"
...
private def treeD_intersectional : IO Unit := do
  ...
  let utrustM : TrustClaim := .untrust .twoSample tBM nBM Denied fBM fWM ciM (σBM ∪ σWM)
  ...
  let utrustF : TrustClaim := .untrust .twoSample tBF nBF Denied fBF fWF ciF (σBF ∪ σWF)
...
private def treeE_monitoring : IO Unit := do
  ...
  if inConstraint Prob.zero ciB then do
    let trustClaimB : TrustClaim := .trust .twoSample tB23 nB23 Denied fB23 fB22 ciB (σB23 ∪ σB22)
    ...
    let utrustClaimB : TrustClaim := .untrust .twoSample tB23 nB23 Denied fB23 fB22 ciB (σB23 ∪ σB22)
  ...
  if inConstraint Prob.zero ciW then do
    let trustClaimW : TrustClaim := .trust .twoSample tW23 nW23 Denied fW23 fW22 ciW (σW23 ∪ σW22)
    ...
    let utrustClaimW : TrustClaim := .untrust .twoSample tW23 nW23 Denied fW23 fW22 ciW (σW23 ∪ σW22)
...
private def printSummary : IO Unit := do
  ...
  IO.println "  ║      Tree C: obs → UPDATE → IEx → EEx (depth 4)          ║"
...
def main : IO Unit := do
  ...
  IO.println ""
  IO.println "─── C. Full chain: UPDATE→IEx→EEx (depth 4) ── showcases (b)(d) ───"
  IO.println ""
  treeC_fullChain
