...
a score-test acceptance-band membership test: if the model probability $p$
...
\caption{Trust-introduction rules.
  ...
  In the implementation the two-sample rules are oriented
  ($f \ge g$); the opposite direction is represented by
  swapping the premises.}
...
\caption{Trust elimination, comparison, and comparison-elimination rules.
  \textsc{ET}, \textsc{EUT}, and \textsc{ET}$^{\text{ex}}$ are
  one-sample eliminations: they are not admissible for the
  two-sample $\Trust_\CItwo$/$\UTrust_\CItwo$, whose interval is
  for a difference $f{-}g$ rather than for the marginal
  ...
  \textsc{IEx}/\textsc{INEx} share the orientation convention
  $f\ge g$ of \textsc{IT2}/\textsc{IUT2}.}
...
The current implementation treats two-sample comparisons as
\emph{oriented positive-gap certificates}: the left premise must
have frequency at least that of the right premise, and the
opposite direction is represented by swapping premises.  Under
this side condition, clamping the two-sample acceptance band to
$[0,1]$ preserves the decision of whether $0$ lies in the
interval.  A full signed-difference type, with intervals in
$[-1,1]$ and explicit deficit certificates, together with
two-sample eliminations into marginal constraints, is left to
future work; in the present fragment, $\textsc{ET}$,
$\textsc{EUT}$, and $\textsc{ET}^{\text{ex}}$ are one-sample
eliminations only.
Other future work includes signed-difference probability handling,
